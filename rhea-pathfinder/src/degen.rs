...
impl DegenSwap {
    pub fn new(amp: u64, degens: &[Balance]) -> Self {
        Self {
            amp: amp as u128,
            degens: degens.to_owned(),
        }
    }

    /// *
    fn mul_degen(&self, amount: Balance, degen: Balance) -> Balance {
        (U384::from(amount) * U384::from(degen) / U384::from(PRECISION)).as_u128()
    }

    /// *
    fn div_degen(&self, amount: Balance, degen: Balance) -> Balance {
        (U384::from(amount) * U384::from(PRECISION) / U384::from(degen)).as_u128()
    }

    /// *
    pub fn degen_balances(&self, amounts: &[Balance]) -> Vec<Balance> {
        amounts
            .iter()
